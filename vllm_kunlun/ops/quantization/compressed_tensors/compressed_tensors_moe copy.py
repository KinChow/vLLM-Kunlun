...
class KunlunCompressedTensorsWNA16MoEMethod(CompressedTensorsWNA16MoEMethod):
    ...
    def apply(
        ...
        _debug_save = os.environ.get("MOE_DEBUG_SAVE", "0") == "1"
        if _debug_save:
            save_dir = "/tmp/moe_test_data"
            os.makedirs(save_dir, exist_ok=True)
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            save_path = os.path.join(save_dir, f"params_{timestamp}.pt")

            input_params = {
                "hidden_states": x.clone().cpu(),
                "router_logits": router_logits.clone().cpu(),
                "w13_weight_packed": layer.w13_weight_packed.clone().cpu(),
                "w2_weight_packed": layer.w2_weight_packed.clone().cpu(),
                "w1_scale": self.moe_quant_config.w1_scale.clone().cpu(),
                "w2_scale": self.moe_quant_config.w2_scale.clone().cpu(),
                "top_k": top_k,
                "renormalize": renormalize,
                "use_grouped_topk": use_grouped_topk,
                "num_expert_group": num_expert_group,
                ...
                "e_score_correction_bias": e_score_correction_bias.clone().cpu() if e_score_correction_bias is not None else None,
            }
            torch.save(input_params, save_path)
            logger.info(f"[MoE Debug] Saved input params to: {save_path}")
        # === 调试保存结束 ===
